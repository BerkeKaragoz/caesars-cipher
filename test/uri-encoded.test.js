...
var _ = require("../dist/uri-encoded");
...
describe("URI Encoded", function () {
  describe("Cipher", function () {
    ...
    it("should shift by one", function () {
      var shiftAmount = 1;
      assert.equal(_.caesarsCipher(shortText, shiftAmount), "Djqifs");
    });

    it("should be able to shift more than the upper boundary", function () {
      var shiftAmount = MAX_LEGAL_RANGE;

      assert.equal(_.caesarsCipher(shortText, shiftAmount), shortText);
    });
    ...
    it("should be able to shift long texts", function () {
      ...
      assert.equal(
        _.caesarsCipher(longText, shiftAmount),
  ...
  describe("Decipher", function () {
    ...
    it("should decipher shift by one", function () {
      var shiftAmount = 1;
      var cipheredText = "Djqifs"; // ciphered shortText by one
    ...
    it("should be able to decipher shifted more than the upper boundary", function () {
      ...
      assert.equal(_.caesarsDecipher(shortText, shiftAmount), shortText);
    });

    it("should be able to shift long texts", function () {
      var shiftAmount = MAX_LEGAL_RANGE;

      assert.equal(_.caesarsDecipher(longText, shiftAmount), longText);
    });
  });
});
